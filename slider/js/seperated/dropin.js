var dropin=function(o){
	...
	m.an=function(cn){
		...
		if(!m.a){
			el.zIndex=1;
			ec.zIndex=1000;ec.display=el.display="block";
			el.left=ec.left="0px";
			if(m.l<m.c){el.top="0px";ec.top=-(2*m.h)+"px";m.ct=-(2*m.h);}
			else{ec.top=-m.h+"px";el.top=-m.h+"px";m.ct=-(m.h);}					
			m.a=1;			
		}
		if(m.l<m.c && m.ct<-m.h){
		...
		else if(m.l>m.c && m.ct<0){
			if(m.ct+m.sh>0){m.ct=0;}else{m.ct+=m.sh;}
			ec.top=m.ct+"px";	
		}
		else{
