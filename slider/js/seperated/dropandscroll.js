var dropandscroll=function(o){
	...
	m.an=function(cn){
		...
		if(!m.a){
			...
			ec.top="0px";ec.left=m.w+"px";
			m.a=1;
		}
		if(Math.abs(m.t)<Math.abs(m.h)){m.t+=m.sh;el.top=m.t+"px";}
		else if(m.cl>0){		
			el.display="none";ec.display="block";
			if(m.cl-m.sw<0){m.cl=0;}else{m.cl-=m.sw;}
			ec.left=m.cl+"px";		
		}		
		else{
			m.cl=m.w;m.t=0;m.a=0;m._e(cn);return;		
		}
